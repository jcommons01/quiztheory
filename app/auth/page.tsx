...
export default function AuthPage() {
  ...
  useEffect(() => {
    const unsub = auth.onAuthStateChanged((user) => {
      if (user) {
        // If arriving with a pending join code, join then go to dashboard
  ...
  // If ?join=CODE is passed, stage it so post-auth can complete the join
  useEffect(() => {
...
function humanizeFirebaseError(err: any): string {
  ...
  // Try to map common auth codes to friendly messages
